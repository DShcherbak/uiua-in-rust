...
pub fn uiua_binary(a: UiuaElements, b: UiuaElements, f : UiuaElements) -> UiuaElements  {
    ...
    match (a,b) {
        (UiuaElements::Elem(lhs), UiuaElements::Elem(rhs)) => {
            UiuaElements::Elem(f(lhs, rhs))
        }
        ...
        (UiuaElements::Vector(lhs), UiuaElements::Vector(rhs)) => {
            if lhs.len() != rhs.len() {
                return UiuaElements::Error(format!("Vector length mismatch: {:?} and {:?}", lhs, rhs).to_string())
            }
            UiuaElements::Vector(lhs.iter().zip(rhs.into_iter()).map(|(x, y)| f(*x, y)).collect())
        }
        (e1, e2) => UiuaElements::Error(format!("Arguments of {} cannot be of types {:?} and {:?}", name, e1, e2))
    }
}
        ...
        loop {
            ...
            for elem in self.chars.iter() {
                match elem {
                    ...
                    UiuaElements::Plus | UiuaElements::Minus | UiuaElements::Mult | UiuaElements::Div => {
                        match reverse_stack.pop() {
                            Some(lhs) => {
                                match reverse_stack.pop() {
                                    Some(rhs) => {
                                    ...
                                    None => return UiuaElements::Error(format!("Not enough arguments for a function {:?}", elem.clone()))
                                }
                            }
                            None => return UiuaElements::Error(format!("Not enough arguments for a function {:?}", elem.clone()))
                        }
